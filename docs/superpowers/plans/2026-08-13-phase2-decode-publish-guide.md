...
1. **Passive validation**: create a dedicated `infinid` user on the operator's Mosquitto broker (per-service credentials; the password lives host-side only and is entered once into add-on options — never in any repo). Deploy add-on v0.2.0 (SAM off), confirm the 12 contract entities + zone 2/3 entities appear in HA and track the wall panel through a setpoint change. Dashboard HvacPage DiagTiles light up with zero app changes.
